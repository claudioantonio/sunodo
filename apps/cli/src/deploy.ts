...
export type RewardsType = "offchain" | "erc20";
...
const configureRewards = async (): Promise<RewardsType> => {
    return selectAuto<RewardsType>({
...
const deploy = async (
    ...
    await configureRewards();

    // send transaction
    const addresses: Record<number, Address> = {
        31337: cartesiIpfsdAppFactoryAddress[31337],
    };
    if (!walletClient.chain) {
        throw new Error("Undefined chain");
    }
    const address = addresses[walletClient.chain.id];
    if (!address) {
        throw new Error(
            `System not deployed on chain ${walletClient.chain.name}`
        );
    }
